 ...
 */
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> = a.intersect(b.asIterable()).toList()
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    list.map { m[it] = m.getOrDefault(it, 0) + 1 }
    return m.filter { it.value > 1 }
}
...
fun hasAnagrams(words: List<String>): Boolean =
    words.map { it.toSortedSet() }.toSet().size != words.size
...
fun propagateHandshakes(friends: Map<String, Set<String>>): MutableMap<String, MutableSet<String>> {
    ...
    for ((key) in friends)
        result[key]!!.addAll(findName(key, result, mutableListOf(key)).minus(key))
    return result
}
...
fun findName(
    ...
    Origin: MutableList<String>
): Collection<String> {
    val res = mutableSetOf<String>()
    for (i in result[skey]!!)
        if (result[i] == null) result[i] = mutableSetOf()
        else if (i in Origin) continue
        else {
            res.addAll(result[i]!!)
            Origin.add(i)
            res.addAll(findName(i, result, Origin))
        }
    return res
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in (0 until list.size)) {
        l.removeAt(0)
        if (number - list[i] in l) return Pair(i, l.indexOf(number - list[i]) + k)
        k++
    }
    return Pair(-1, -1)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    val t = treasures.filter { it.value.first <= capacity }
    if (t.isEmpty()) return emptySet()
    val v = IntArray(capacity + 1) { 0 }
    val s = Array<Set<String>>(capacity + 1) { emptySet() }
    (t.values.minBy { it.first }!!.first..capacity).forEach { weight ->
        var mxv = v[weight - 1]
        var mxs = s[weight - 1]
        t.forEach { (g, p) ->
            val it = weight - p.first
            if (it >= 0 && g !in s[it]) {
                val sv = p.second + v[it]
                if (sv > mxv) {
                    mxv = sv
                    mxs = s[it] + g
                }
